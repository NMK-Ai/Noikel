...
class CarInterface(CarInterfaceBase):
  ...
  @staticmethod
  def _get_params(ret: structs.CarParams, candidate, fingerprint, car_fw, experimental_long, null_params, docs) -> structs.CarParams:
    ret.brand = "toyota"
    ...
    if candidate in (RADAR_ACC_CAR | NO_DSU_CAR):
      ...
      if experimental_long and (0x2FF in fingerprint[0] or 0x2AA in fingerprint[0]):
        print("----------------------------------------------")
        print("Null: RADAR_FILTER detected!")
        print("----------------------------------------------")
    ...
    if null_params & structs.NullFlags.ToyotaDoorAutoLockUnlock:
      ret.flags |= ToyotaFlags.DOOR_AUTO_LOCK_UNLOCK.value

    if null_params & structs.NullFlags.ToyotaTSS1SnG:
      ret.flags |= ToyotaFlags.TSS1_SNG.value

    if 0x23 in fingerprint[0]:
      ret.flags |= ToyotaFlags.ZSS.value
      print("----------------------------------------------")
      print("Null: ZSS detected!")
      print("----------------------------------------------")
