...
class Car:
  ...
  def __init__(self, CI=None, RI=None) -> None:
    ...
    if CI is None:
      ...
      if self.params.get_bool("null_lateral_alka"):
        null_params |= structs.NullFlags.LateralALKA

      if self.params.get_bool("null_toyota_door_auto_lock_unlock"):
        null_params |= structs.NullFlags.ToyotaDoorAutoLockUnlock

      if self.params.get_bool("null_toyota_tss1_sng"):
        null_params |= structs.NullFlags.ToyotaTSS1SnG

      if self.params.get_bool("null_vag_a0_sng"):
        null_params |= structs.NullFlags.VagA0SnG

      self.CI = get_car(*self.can_callbacks, obd_callback(self.params), experimental_long_allowed, num_pandas, null_params, cached_params)
    ...
    if null_params & structs.NullFlags.LateralALKA:
